...
def test_learning_runtime():
    for n in range(1, 4):
        ...
        assert elapsed_time < 0.01, f"Learning is too slow: {elapsed_time:.4f} seconds"

@pytest.mark.ngram_4
def test_log_probability_runtime():
    model = NGramLM(3)
    model.learn(DATA)
    ...
    for sequence in test_sequences:
        ...
        assert elapsed_time < 0.001, f"log_probability is too slow for input length {len(sequence)}"
...
def test_generate_runtime():
    ...
    start_time = time.time()
    model.generate(num_samples=10, max_steps=20, results_file="test_output.txt")
    elapsed_time = time.time() - start_time

    assert elapsed_time < 0.05, f"Text generation too slow: {elapsed_time:.4f} seconds"
